...
> [!warning] Cuidado con el almacenamiento
...
> [!faq]- ¿Qué switch inicia captura sin resolución de nombres, verbose, en ASCII+hex, y captura los primeros 100 paquetes?
> ```bash
> sudo tcpdump -nn -v -X -c 100
> ```

> [!faq]- Comando para leer `/tmp/capture.pcap` mostrando hex y ASCII (buenas prácticas)
> ```bash
> sudo tcpdump -r /tmp/capture.pcap -X
> ```

> [!faq]- ¿Qué switch aumenta la verbosidad?
> `-v` (o `-vv`, `-vvv` para más detalle)

> [!faq]- ¿Qué referencia de ayuda del terminal da más info sobre tcpdump?
> `man tcpdump`

> [!faq]- ¿Qué switch permite escribir la salida a un archivo?
> `-w`

## 🔗 Relacionado
- [[05-analysis-practice]]
- [[07-tcpdump-lab-fundamentals]]
